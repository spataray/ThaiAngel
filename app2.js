...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const userDisplay = document.getElementById('user-display'); // Renamed for clarity
    ...
    // Safely gets the username from the synthetic email
    ...
    onAuthStateChanged(auth, user => {
        if (user) { // User is logged in
            ...
            initializeDataListeners(user.uid); // Pass UID for potential future use
        } else { // User is logged out
            authContainer.classList.remove('hidden');
            mainContent.classList.add('hidden');
            userDisplay.textContent = '';
            // Detach listeners to prevent errors when logged out
            off(ref(db, 'products'));
            off(ref(db, 'requests'));
        }
    });
